...
export const PlaylistCard = ({
  ...
  return (
    <Link
      className="group"
      href={`playlist/${id}?name=${encodeURIComponent(playlistName)}`}
    >
      <Card className=" px-4 cursor-pointer hover:bg-surface-hover transition-all duration-400 hover:-translate-y-1.5">
        <div className="overflow-hidden rounded-md relative w-auto h-auto">
          ...
          <Image
            ...
            alt={playlistName}
            src={playlistImage}
            className="rounded-md w-full h-auto group-hover:scale-110 group-hover:blur-xs transition-all duration-300 object-cover"
          />
        </div>
