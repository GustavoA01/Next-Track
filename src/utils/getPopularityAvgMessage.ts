...
export const getPopularityAvgMessage = (tracks: SpotifyPlaylistTracks, playlistLength: number): AverageMessageType => {
  const sum = tracks.items.reduce((sum, item) => sum + item.track.popularity, 0);
